...
from models import master_bot
from Utils import logs

app = Flask(__name__)
master = master_bot.start()
...
def getQr():
    try:
        qr = master.initSession()
        return send_from_directory(configAPI.pathFiles,qr)
    except Exception:
        logs.logError('Master-API',traceback.format_exc())
        return Response(json.dumps(master_bot._Responses["500"]), mimetype='application/json')
